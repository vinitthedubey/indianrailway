...
def train_booking(train_details,passanger_detail,other_detail,language_input):
    try:
        ...
        if(len(train_details)!=10):
            return (translator.translate("Error while accessing train details Refresh",src="en",dest=language_input).text)
    
        if(passanger_detail[0]=="" or passanger_detail[0]==None or (str(passanger_detail[1]).isdigit()==False)or passanger_detail[1]=="" or passanger_detail==None or passanger_detail[2]=="" or passanger_detail[2]==None or passanger_detail[3]=="" or passanger_detail[3]==None):
...
#,translator.translate(class_size,src="en",dest=language_input).text,translator.translate(index,src="en",dest=language_input).text,translator.translate(seat_store,src="en",dest=language_input).text])
            ...
            traindata={"train_number":train_details[0],"train_name":translator.translate(train_details[1],src=language_input,dest="en").text,"from_station_name":translator.translate(train_details[2],src=language_input,dest="en").text,
